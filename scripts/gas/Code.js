...
function handleFinalize_(payload) {
  ...
  receipts.forEach(r => {
    try {
      const receiptKey = r.driveFileId
        || `finalize_${Date.now()}_${generateRandId_(6)}`;
      if (!r.refinalize && isAlreadyFinalized_(receiptKey)) {
        throw { code: 'ALREADY_FINALIZED', status: 409,
                message: `${receiptKey} は既にfinalizeされています` };
      }
      const normalized = normalizeReceipt_(r, r);
      normalized.receipt_key = receiptKey;
      ...
      entries.forEach(entry => {
        ...
        appendRow_(getSheet_('経費'), built, receiptKey, 'finalize');
      });
      if (r.driveFileId) {
        const newName = buildFileName_(reasoned);
        renameAndMoveFile_(r.driveFileId, newName);
      }
      auditLog_('finalize', receiptKey, r, reasoned);
      runGlobalReview_('light');
      results.push({
        receiptKey,
        fileName: r.driveFileId ? buildFileName_(reasoned) : null,
      ...
      results.push({
        error: err.code || 'FINALIZE_FAILED',
        message: err.message
      });
    }
  });
...
function renameAndMoveFile_(fileId, newName) {
  try {
    const props = PropertiesService.getScriptProperties();
    const file = DriveApp.getFileById(fileId);
    file.setName(newName);
    const targetFolder = getDriveFolder_(
      props.getProperty('DRIVE_FOLDER_ID'));
    file.moveTo(targetFolder);
  } catch (e) {
    Logger.log(`renameAndMoveFile_ error: ${e.message}`);
  }
}
